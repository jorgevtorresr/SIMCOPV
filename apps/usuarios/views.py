...
from .serializers import (UserSerializer, PersonaSerializer, TipoNotificacionSerializer, NotificacionSerializer,
                          PeriodoSerializer)
            ...
            for user in lista:
                ...
                if tmp.persona.tipo == "LOSEP":
                    ...
                    try:
                        ...
                        pass
                    if periodomax == []:
                        periodo = Periodo(anio_periodo=year, dias_fijo=15, dias_vac=15, horas_vac=dt.time(0),
                                          usuario=tmp)
                        periodo.save()
                    else:
                        dias = 30 if periodomax.dias_fijo + 1 > 30 else periodomax.dias_fijo + 1
                        periodo = Periodo(anio_periodo=year, dias_fijo=dias, dias_vac=dias, horas_vac=dt.time(0),
                                          usuario=tmp)
                        periodo.save()
...
def creacionderoles():
    ...
    validarvacaciones, p4 = GlobalPermission.objects.get_or_create(codename="validar_vacaciones",
                                                                   name="Validar Vacaciones")
    registrarusuarios, p5 = GlobalPermission.objects.get_or_create(codename="registrar_usuarios",
    ...
    jefesdetalento.permissions = (
    validarvacaciones, validarpermiso, registrarusuarios, cambiarpassword, pedirpermiso, generarreportes)
...
def ajax_usuarios_ver(request):
    if request.is_ajax():
        ...
        ajax = User.objects.filter(last_name__icontains=query, is_superuser=False, is_active=True).order_by(
            "persona__unidad", "last_name", "first_name")
        try:
        ...
        if request.POST.get("unidad", "") == "unidad":
            ...
            if form_unidad.is_valid():
                ...
                return render(request, 'usuarios/agregarusuario.html',
                              {"form": form, "form_unidad": form_unidad, "men": men})
        else:
            ...
            if form.is_valid():
                ...
                if verificar(cedula):
                    ...
                    if existe:
                        ...
                        usuario = User.objects.create_user(username=cedula, first_name=nombre, last_name=apellido,
    ...
    if request.method == "POST":
        ...
        if image1 == "" or image2 == "" or image3 == "" or image4 == "":
            ...
            if a or b or c or d:
                ...
                dic = {"imagen": image1._get_name(), "image2": image2._get_name(), "image3": image3._get_name(),
                       "image4": image4._get_name()}
